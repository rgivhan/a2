...
def naiveSoftmaxLossAndGradient(
    ...
    y_bar = softmax(np.dot(outsideVectors,centerWordVec))
    y0_bar = y_bar[outsideWordIdx]
    #Get y as one-hot
    y = np.zeros(y_bar.shape)
    y[outsideWordIdx] = 1
    #compute loss & grads
    loss = -np.log(y0_bar)
...
def negSamplingLossAndGradient(
    ...
    u_ws = outsideVectors[negSampleWordIndices] 
    sigma_o = sigmoid(np.dot(u0, centerWordVec))
    sigma_ws = sigmoid(-1 * np.dot(u_ws,centerWordVec)) 

    loss = -np.log(sigma_o) - np.sum(np.log(sigma_ws))

    gradCenterVec += -u0 * (1.0 - sigma_o) + np.dot(u_ws.T, (1-sigma_ws))
    gradOutsideVecs[outsideWordIdx] += -(1.0 - sigma_o) * centerWordVec

    for j in range(K):
        idx = negSampleWordIndices[j]
        u_ws = outsideVectors[idx]
        sigma_ws = sigmoid(np.dot(-u_ws, centerWordVec))
        gradOutsideVecs[idx] += centerWordVec * (1.0 - sigma_ws)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for o in outsideWords:
        o_idx = word2Ind[o]
        (loss_out, gradCenter, gradOutside) = word2vecLossAndGradient(v_c, o_idx, outsideVectors, dataset)
        loss += loss_out
        gradCenterVecs[c_idx] += gradCenter
